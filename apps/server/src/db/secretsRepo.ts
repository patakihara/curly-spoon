/**
 * Per-user upstream credentials, encrypted at rest (db/crypto.ts). Nothing in this
 * file's return types ever includes the ciphertext or plaintext together — callers
 * get either "store this" or "here is the decrypted token", never both at once.
 */
...
export function deleteUpstreamToken(db: Db, userId: string): void {
  db.prepare('DELETE FROM secrets WHERE user_id = ?').run(userId);
}
