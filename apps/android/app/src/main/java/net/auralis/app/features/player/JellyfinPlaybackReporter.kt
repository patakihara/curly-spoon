 ...
 * `net.auralis.app.features.music.albumPlaybackQueue`, the one place that constructs one
 * (`"track:${track.id}"`). Distinct from `net.auralis.app.playback.PlaybackItemResolver`'s own
 ...
 * make. A plain interface, not `net.auralis.app.features.music.MusicRepository` directly, so
 ...
 * isolating untestable Android glue (`net.auralis.app.playback.MediaItemConversions`,
 * `net.auralis.app.playback.PlaybackItemResolver`) from the decision logic that drives it.
