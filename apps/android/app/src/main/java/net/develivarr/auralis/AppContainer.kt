...
import net.develivarr.auralis.data.downloads.Media3DownloadEngine
import net.develivarr.auralis.data.network.ApiClient
import net.develivarr.auralis.data.network.ApiException
import net.develivarr.auralis.data.network.DataStoreKeyValueStore
...
class AppContainer(context: Context) {
    ...
    /**
     * Wired through to [net.develivarr.auralis.features.player.PlayerViewModel], which builds the
     * actual [net.develivarr.auralis.features.player.JellyfinPlaybackReporter] around this — see that
    ...
    /**
     * Shared by [net.develivarr.auralis.features.player.PlayerViewModel] (the phone UI's "tap a shelf
     * item" path) and [net.develivarr.auralis.playback.AuralisMediaLibraryService] (its own, separately
     ...
     * [net.develivarr.auralis.playback.AuralisMediaLibraryService] (for read-only playback), so a
     ...
     * [net.develivarr.auralis.playback.AuralisDownloadService] (owns the actual download work) — both
     ...
     * interface plus [net.develivarr.auralis.data.downloads.UnavailableDownloadEngine], a no-op
